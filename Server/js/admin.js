document.addEventListener("DOMContentLoaded", () => {
    ...
    // /* ── Dynamic breadcrumb ── */
    // function setPage(section, subItems, clickedEl) {
    //     // active state
    //     document
    ...
    //     // title
    //     const title = subItems.length ? subItems[subItems.length - 1] : section;
    //     document.getElementById("pageTitle").textContent = title;

    //     // breadcrumb
    //     const bc = document.getElementById("breadcrumb");
    //     let html = `<li class="breadcrumb-item"><a href="#"><i class="fa fa-house" style="font-size:.75rem;"></i></a></li>`;
    //     if (section)
    //         html += `<li class="breadcrumb-item${subItems.length === 0 ? " active" : ""}">
    //   ${subItems.length ? `<a href="#">${section}</a>` : section}</li>`;
    //     subItems.forEach((s, i) => {
    //         const last = i === subItems.length - 1;
    //         html += `<li class="breadcrumb-item${last ? " active" : ""}">${last ? s : `<a href="#">${s}</a>`}</li>`;
    //     });
    //     bc.innerHTML = html;
    // }
    // $("#nev-menu-items").click(function (e) {
    //     e.preventDefault();
    //     let title = $(this).data("title");
    //     let subItems = $(this).data("subitems");
    //     setPage(title, subItems, this);
    // });
});
